<?php
$conn = new mysqli("localhost", "root", "", "tarun");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($address)) {
        ...
        if (empty($orderedBooks)) {
            ...
            $bookCodes = implode("','", array_keys($orderedBooks));
            $result = $conn->query("SELECT id, code FROM books WHERE code IN ('$bookCodes')");
            $bookIdMap = [];
            while ($row = $result->fetch_assoc()) {
                $bookIdMap[$row['code']] = $row['id'];
            }

            // Insert order items
            $stmt = $conn->prepare("INSERT INTO order_items (order_id, book_id, quantity) VALUES (?, ?, ?)");
            foreach ($orderedBooks as $code => $qty) {
                $book_id = $bookIdMap[$code];
            ...
            echo "<ul>";
            foreach ($orderedBooks as $code => $qty) {
                $bookName = $books[$code]['name'];
                $price = $books[$code]['price'];
                $subtotal = $qty * $price;
                echo "<li>$bookName x $qty = ₹$subtotal</li>";
            }
            echo "</ul>";
